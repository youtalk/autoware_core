...
(``config/interface_gates.yaml``); M2.1 ratchets the manifest / honesty / isolation
gates to ``error`` so a finding fails the build. ``--warn-only`` overrides every
gate to exit 0 for local advisory runs.
"""
