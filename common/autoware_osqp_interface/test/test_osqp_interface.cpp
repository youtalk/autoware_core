...
// The pure message-builder backing logUnsolvedStatus covers every branch:
// solved (status == 1) -> nullopt, unsolved without/with prefix.
TEST(TestOsqpInterface, BuildUnsolvedStatusMessage)
{
  using autoware::osqp_interface::detail::build_unsolved_status_message;

  // status == 1 means solved: no message.
  ...
  // Unsolved without a prefix.
  {
    const auto msg = build_unsolved_status_message(-3, "primal infeasible", "");
    ASSERT_TRUE(msg.has_value());
    EXPECT_EQ(*msg, "Optimization failed due to primal infeasible");
  }

  // Unsolved with a prefix (a single space separates prefix and body).
  {
    const auto msg = build_unsolved_status_message(-4, "dual infeasible", "[MPC]");
    ASSERT_TRUE(msg.has_value());
    EXPECT_EQ(*msg, "[MPC] Optimization failed due to dual infeasible");
  }
}

// logUnsolvedStatus is the thin wrapper around the builder; after a solved problem it must be a
// no-op (status == 1 -> nullopt) and must never throw.
