...
- QoS overrides validated against a spec's compatibility pivot
...
`create_publisher<Spec>()` and `create_subscription<Spec>(callback)` use the spec's own declared QoS.
`create_publisher<Spec>(const rclcpp::QoS &)` and `create_subscription<Spec>(callback, const rclcpp::QoS &)` accept an explicit override instead, validated against the spec's compatibility pivot before anything is constructed or registered: a provider must offer at least the pivot, and a consumer must request at most the pivot.
Depth is not part of that comparison and is always free to override.
A policy the pivot cannot rank against at all -- for example `rclcpp::SystemDefaultsQoS()` or `BestAvailableQoS()` -- is rejected too, the same as an outright weaker offer or stronger request: an incomparable override fails closed rather than passing silently.

A violation throws `QosContractViolation` (`autoware/component_interface_utils/rclcpp/exceptions.hpp`) before the publisher or subscription is created, so a non-conforming node fails at construction instead of running with a QoS nothing admitted.
