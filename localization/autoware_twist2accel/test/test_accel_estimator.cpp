...
// The node fills the accel covariance diagonal with the historical magic
// indices 0,7,14,21,28,35. Lock in that those equal the XYZRPY layout so the
// switch to named indices is provably equivalent.
TEST(AccelEstimatorTest, CovarianceDiagonalIndicesMatchXyzrpyLayout)
{
  EXPECT_EQ(static_cast<int>(XYZRPY_COV_IDX::X_X), 0 * 6 + 0);
  EXPECT_EQ(static_cast<int>(XYZRPY_COV_IDX::Y_Y), 1 * 6 + 1);
  EXPECT_EQ(static_cast<int>(XYZRPY_COV_IDX::Z_Z), 2 * 6 + 2);
  EXPECT_EQ(static_cast<int>(XYZRPY_COV_IDX::ROLL_ROLL), 3 * 6 + 3);
  EXPECT_EQ(static_cast<int>(XYZRPY_COV_IDX::PITCH_PITCH), 4 * 6 + 4);
  EXPECT_EQ(static_cast<int>(XYZRPY_COV_IDX::YAW_YAW), 5 * 6 + 5);
}
