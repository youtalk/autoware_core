...
TEST(GyroOdometerFusion, DetermineDiagnosticsWarnWhenNotArrived)
{
  ...
  EXPECT_EQ(result.entries[0].message, "Twist msg has not been arrived yet.");
  EXPECT_EQ(result.entries[1].message, "IMU msg has not been arrived yet.");
  EXPECT_EQ(
    result.log_message, "Twist msg has not been arrived yet.; IMU msg has not been arrived yet.; ");
}

// determine_diagnostics: a timeout raises ERROR and the message text matches the historical format.
TEST(GyroOdometerFusion, DetermineDiagnosticsErrorOnTimeout)
{
  ...
  EXPECT_EQ(
    ...
    "Vehicle twist msg is timeout. vehicle_twist_dt: 2[sec], tolerance 1[sec]");
}

// determine_diagnostics: TF failure raises ERROR and the message embeds the configured output
// frame.
TEST(GyroOdometerFusion, DetermineDiagnosticsErrorOnTransformFailure)
{
  ...
  EXPECT_EQ(result.level, diagnostic_msgs::msg::DiagnosticStatus::ERROR);
  ASSERT_EQ(result.entries.size(), 1u);
  EXPECT_EQ(result.entries[0].message, "Please publish TF from base_link to frame of IMU.");
}

// determine_diagnostics: when both WARN and ERROR conditions trigger, the aggregated level is the
// ERROR maximum, while every entry keeps its own level and message order.
TEST(GyroOdometerFusion, DetermineDiagnosticsAggregatesToMaxSeverity)
{
  ...
  EXPECT_EQ(result.entries[1].message, "Please publish TF from base_link to frame of IMU.");
}
